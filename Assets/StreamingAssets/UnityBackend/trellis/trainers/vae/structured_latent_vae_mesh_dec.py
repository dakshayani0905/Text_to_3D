...
class SLatVaeMeshDecoderTrainer(BasicTrainer):
    ...
    def _tsdf_reg_loss(rep: MeshExtractResult, depth_map: torch.Tensor, extrinsics: torch.Tensor, intrinsics: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            projected_pts, pts_depth = utils3d.project_cv(extrinsics=extrinsics, intrinsics=intrinsics, points=rep.tsdf_v)
    ...
    def _flip_normal(self, normal: torch.Tensor, extrinsics: torch.Tensor, intrinsics: torch.Tensor) -> torch.Tensor:
        ...
        R[:, 3, 3] = 1.0
        view_dir = utils3d.unproject_cv(
            utils3d.image_uv(*normal.shape[-2:], device=self.device).reshape(1, -1, 2),
    ...
    def run_snapshot(
        ...
        for yaw, pitch in zip(yaws, pitch):
            ...
            extrinsics = utils3d.extrinsics_look_at(orig, torch.tensor([0, 0, 0]).float().cuda(), torch.tensor([0, 0, 1]).float().cuda())
            intrinsics = utils3d.intrinsics_from_fov_xy(fov, fov)
