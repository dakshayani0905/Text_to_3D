...
def _fill_holes(
    ...
    projection = utils3d.perspective_from_fov_xy(fov, fov, 1, 3)
    views = []
    for (yaw, pitch) in zip(yaws, pitchs):
        ...
        view = utils3d.view_look_at(orig, torch.tensor([0, 0, 0]).float().cuda(), torch.tensor([0, 0, 1]).float().cuda())
        views.append(view)
    views = torch.stack(views, dim=0)

    # Rasterize
    visblity = torch.zeros(faces.shape[0], dtype=torch.int32, device=verts.device)
    rastctx = utils3d.RastContext(backend='cuda')
    print(type(rastctx))
    print(dir(rastctx))
    for i in tqdm(range(views.shape[0]), total=views.shape[0], disable=not verbose, desc='Rasterizing'):
        view = views[i]
        buffers = utils3d.rasterize_triangle_faces(
    ...
    edges, face2edge, edge_degrees = utils3d.compute_edges(faces)
    boundary_edge_indices = torch.nonzero(edge_degrees == 1).reshape(-1)
    connected_components = utils3d.compute_connected_components(faces, edges, face2edge)
    ...
    dual_edges, dual_edge2edge = utils3d.compute_dual_graph(face2edge)
    ...
    to_remove_cc = utils3d.compute_connected_components(faces[remove_face_indices])
    if debug:
    ...
    for cc in to_remove_cc:
        ...
        if len(cc_new_boundary_edge_indices) > 0:
            cc_new_boundary_edge_cc = utils3d.compute_edge_connected_components(edges[cc_new_boundary_edge_indices])
    ...
    if len(valid_remove_cc) > 0:
        ...
        faces = faces[mask]
        faces, verts = utils3d.remove_unreferenced_vertices(faces, verts)
        if verbose:
...
def bake_texture(
    ...
    views = [utils3d.extrinsics_to_view(torch.tensor(extr).cuda()) for extr in extrinsics]
    projections = [utils3d.intrinsics_to_perspective(torch.tensor(intr).cuda(), near, far) for intr in intrinsics]
    ...
    if mode == 'fast':
        ...
        for observation, view, projection in tqdm(zip(observations, views, projections), total=len(observations), disable=not verbose, desc='Texture baking (fast)'):
            with torch.no_grad():
                rast = utils3d.rasterize_triangle_faces(
        ...
        mask = 1 - utils3d.rasterize_triangle_faces(
...
def simplify_gs(
    ...
    with tqdm(total=500, disable=not verbose, desc='Simplifying Gaussian') as pbar:
        for i in range(500):
            if i % 50 == 0:
                print(f"Progress: {i}/500 ({i/500*100:.1f}%)")
            # prune
            if i % 100 == 0:
            ...
            for j in range(len(optimizer.param_groups)):
                optimizer.param_groups[j]['lr'] = cosine_anealing(optimizer, i, 500, start_lr[j], end_lr[j])
